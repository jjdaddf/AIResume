  ...
  <div class="bg-white shadow-lg rounded-lg w-[210mm] min-h-[297mm]" :style="rootStyle">
    ...
    <template v-else-if="isSidebar">
      <div class="flex" style="min-height: 297mm;">
        <!-- 侧边栏 -->
        <div
          class="w-[35%] flex-shrink-0 py-8 px-5"
          :style="{ backgroundColor: t.sidebarBg, color: t.sidebarColor, padding: `${store.layout.margin_top}mm 5mm ${store.layout.margin_bottom}mm 5mm` }"
          :class="t.pageLayout === 'sidebar-right' ? 'order-2' : 'order-1'"
        >
        ...
        <!-- 主内容区 -->
        <div
          class="flex-1 py-8 px-7"
          :class="t.pageLayout === 'sidebar-right' ? 'order-1' : 'order-2'"
          :style="{ padding: `${store.layout.margin_top}mm 7mm ${store.layout.margin_bottom}mm 7mm` }"
        >
...
export default {
  components: { SectionHeading, SectionBody },
}
</script>
